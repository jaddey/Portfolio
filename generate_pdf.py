...
<head>
    ...
    <style>
        ...
        *, *::before, *::after {{ box-sizing: border-box; }}
        ...
        .item {{ margin-bottom: 12px; padding-bottom: 10px; border-bottom: 1px dashed rgba(255, 255, 255, 0.08); }}
        .item:last-child {{ border-bottom: none; margin-bottom: 0; padding-bottom: 0; }}
        .item-header {{ display: table; width: 100%; }}
        .item-title {{ display: table-cell; font-weight: bold; color: #ffffff; }}
        .item-date {{ display: table-cell; text-align: right; color: #9ca3af; font-size: 8.5pt; }}
        .item-subtitle {{ color: #148f93; font-size: 9.5pt; margin-bottom: 4px; }}
        .item-desc {{ white-space: pre-line; color: #d1d5db; font-size: 9pt; }}
...
<body>
    <div class="header">
        {"<div class='photo-cell'><img src='" + photo_b64 + "'></div>" if photo_b64 else ""}
    ...
    <div class="section">
        ...
        <div style="white-space: pre-line; color: #9ca3af; font-size: 9.5pt;">{data['aboutText']}</div>
    </div>
    <div class="section">
